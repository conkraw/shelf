...
def create_new_exam(full_df):
    """
    Creates a new exam session from the passed-in full_df.
    Assumes pending question has already been injected if appropriate.
    """
    # Step 1: Filter out recently used questions
    used_ids = get_global_used_questions()
    full_df = full_df[~full_df["record_id"].isin(used_ids)]

    # Step 2: Determine if a recommended question is present (flagged)
    recommended_question = None
    if "recommended_flag" in full_df.columns:
        flagged = full_df[full_df["recommended_flag"] == True]
        if not flagged.empty:
            recommended_question = flagged.iloc[[0]].copy()
            # Drop from main set to avoid duplicates
            full_df = full_df.drop(flagged.index[0])

    # Step 3: Sample remaining questions
    ...
    # Step 4: Add back the recommended question at the top (if any)
    ...
    # Step 6: Store exam session state
